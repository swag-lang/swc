...
<?php include('common/start-head.php'); ?><title>Flappy</title>
...
<div class="code-block"><span class="SCde">$ swag flappy.swgs
</span></div>
...
<p>This page has been generated with Swag directly from the <a href="https://github.com/swag-lang/swag/blob/master/bin/examples/scripts/flappy.swgs">source code</a>.</p>
</div>
<p>So, let's begin.</p>
<h3 id="Dependencies">Dependencies</h3>
<p>Typically, you'd put module setup directly inside the <span class="code-inline">module.swg</span> file of a module. However, if you're working with a script and there isn't a <span class="code-inline">module.swg</span> file, simply put the <span class="code-inline">#dependencies</span> block at the beginning of the script file.</p>
...
<div class="code-block"><span class="SCde"><span class="SFct">#dependencies</span>
{
    ...
    <span class="SFct">#run</span>
    {
        ...
        <span class="SKwd">let</span> cfg = <span class="SKwd">notnull</span> itf.<span class="SFct">getBuildCfg</span>()
        ...
        <span class="SCmp">#static</span> <span class="SLgc">if</span> <span class="SCmp">#cfg</span> == <span class="SStr">"debug"</span> <span class="SLgc">do</span>
            cfg.safetyGuards = <span class="SCst">Swag</span>.<span class="SCst">SafetyWhat</span>.<span class="SCst">All</span>
        <span class="SLgc">else</span> <span class="SLgc">do</span>
            cfg.safetyGuards = <span class="SCst">Swag</span>.<span class="SCst">SafetyWhat</span>.<span class="SCst">None</span>
    }
}
...
<div class="blockquote blockquote-note">
<div class="blockquote-title-block"><span class="blockquote-title">Note</span></div>
<p>You might observe that there's no need to end the lines with a <span class="code-inline">;</span> like in C or C++. While it's still possible, this is not mandatory.</p>
</div>
...
<div class="code-block"><span class="SCde"><span class="SKwd">using</span> <span class="SCst">Core</span>, <span class="SCst">Pixel</span>, <span class="SCst">Gui</span>

</span></div>
<h3 id="Entry_point">Entry point</h3>
<p>The <span class="code-inline">#main</span> entry-point hook is usually the entry point of an executable. It's equivalent to the well known <span class="code-inline">main()</span> function, but without arguments. In the case of a script, this hook will be executed by the compiler as the program entry point.</p>
<div class="blockquote blockquote-note">
<div class="blockquote-title-block"><span class="blockquote-title">Note</span></div>
<p>You might observe that the arrangement of global declarations doesn't make a difference, as we're using the <span class="code-inline">onEvent</span> function before even defining it. Swag does not bother about the global declaration order.</p>
</div>
<div class="code-block"><span class="SCde"><span class="SFct">#main</span>
{
    ...
    <span class="SKwd">expect</span> <span class="SCst">Audio</span>.<span class="SFct">createEngine</span>()
    ...
    <span class="SCmt">// When the script is run with '--arg:swag.test', 'runSurface' automatically exits after a few</span>
    <span class="SCmt">// frames, which is usefull for batch testing.</span>
    <span class="SCst">Application</span>.<span class="SFct">runSurface</span>(<span class="SNum">100</span>, <span class="SNum">100</span>, <span class="SNum">254</span>, <span class="SNum">481</span>, title: <span class="SStr">"Flappy Bird"</span>, hook: &amp;onEvent)
}

</span></div>
<h3 id="Global_definitions">Global definitions</h3>
...
<span class="SKwd">const</span> <span class="SCst">BirdImpulseY</span> = <span class="SNum">350</span>     <span class="SCmt">// 350 is an integer, so the type of BirdImpulseY is 's32'</span>

</span></div>
<h4 id="Variables">Variables</h4>
<div class="code-block"><span class="SCde">
<span class="SKwd">var</span> g_Bird: <span class="SCst">Bird</span>     <span class="SCmt">// 'Bird' is a structure, and will be defined later</span>
...
<div class="code-block"><span class="SCde">
<span class="SKwd">var</span> g_Pipes: <span class="SFct">Array</span>'<span class="SCst">Pipe</span>
...
<span class="SKwd">var</span> g_SoundFlap:  <span class="SCst">Audio</span>.<span class="SCst">SoundFile</span>

</span></div>
<p><span class="code-inline">g_Font</span> is a <b>value</b> pointer to a Font structure.</p>
<div class="code-block"><span class="SCde">
<span class="SKwd">var</span> g_Font: <span class="SItr">#null</span> *<span class="SCst">Font</span>
...
<div class="code-block"><span class="SCde">
<span class="SCmt">// Defines the Bird</span>
...
<h3 id="The_actual_code">The actual code</h3>
...
<div class="code-block"><span class="SCde"><span class="SKwd">func</span> <span class="SFct">onEvent</span>(wnd: *<span class="SCst">Wnd</span>, evt: <span class="SCst">IEvent</span>)-&gt;<span class="STpe">bool</span>
{
    ...
    <span class="SLgc">case</span> <span class="SCst">CreateEvent</span>:
        ...
        <span class="SKwd">expect</span> <span class="SFct">loadAssets</span>(wnd)
...
<div class="code-block"><span class="SCde"><span class="SKwd">func</span> <span class="SFct">paint</span>(painter: *<span class="SCst">Painter</span>)
{
    ...
    <span class="SCmt">// 'for' iterates on all the pipes stored in the dynamic array, and returns each value</span>
    <span class="SCmt">// as a pointer/reference.</span>
    <span class="SLgc">for</span> pipe <span class="SLgc">in</span> g_Pipes
    {
    ...
    <span class="SLgc">if</span> !g_FirstStart
    {
        painter.<span class="SFct">drawStringCenter</span>(g_Rect.<span class="SFct">horzCenter</span>(), <span class="SNum">50</span>, <span class="SCst">Format</span>.<span class="SFct">toString</span>(<span class="SStr">"%"</span>, g_Score), <span class="SKwd">notnull</span> g_Font, <span class="SCst">Argb</span>.<span class="SCst">White</span>)
    }
...
<div class="code-block"><span class="SCde"><span class="SKwd">func</span> <span class="SFct">input</span>(wnd: *<span class="SCst">Wnd</span>)
{
    ...
    <span class="SLgc">if</span> kb.<span class="SFct">isKeyJustPressed</span>(.<span class="SCst">Up</span>)
    {
        ...
        <span class="SKwd">expect</span> <span class="SCst">Audio</span>.<span class="SCst">Voice</span>.<span class="SFct">play</span>(&amp;g_SoundFlap)
    }
}
...
<div class="code-block"><span class="SCde"><span class="SKwd">func</span> <span class="SFct">move</span>()
{
    ...
    <span class="SLgc">if</span> g_Start
    {
        <span class="SLgc">for</span> &amp;pipe <span class="SLgc">in</span> g_Pipes
        {
            ...
            {
                <span class="SKwd">expect</span> <span class="SCst">Audio</span>.<span class="SCst">Voice</span>.<span class="SFct">play</span>(&amp;g_SoundHit)
            ...
            {
                <span class="SKwd">expect</span> <span class="SCst">Audio</span>.<span class="SCst">Voice</span>.<span class="SFct">play</span>(&amp;g_SoundScore)
    ...
    <span class="SLgc">if</span> g_GameOver <span class="SLgc">do</span>
        <span class="SKwd">expect</span> <span class="SCst">Audio</span>.<span class="SCst">Voice</span>.<span class="SFct">play</span>(&amp;g_SoundDie)
}
...
<p>Load all assets.</p>
<p>You can notice that the function has <span class="code-inline">fail</span> after its declaration. This indicated that it can return some errors, and that the caller will have to deal with it.</p>
<div class="code-block"><span class="SCde"><span class="SKwd">func</span> <span class="SFct">loadAssets</span>(wnd: *<span class="SCst">Wnd</span>) <span class="SKwd">fail</span>
{
