...
### F-004 — A conditional's selected call-result temporary is never dropped

- Area: compiler
- Found while: merging the eager call-result cleanup after F-080 made conditional bindings
  deep-copy their selected struct arm.
- Observation: eager call-result temporaries are now dropped at the end of their consuming
  statement, but registration deliberately stands down inside lazy regions. Consequently,
  `let value = cond ? makeThing() : existing` deep-copies the selected temporary arm and abandons
  that temporary with its resources.
- Evidence: `CodeGen::registerTemporaryDrop` returns when its ancestor walk reaches an
  `AstNodeId::ConditionalExpr`, so the generic statement-end flush never sees the selected call.
  Registering it unconditionally would instead drop an unwritten slot whenever the other arm wins.
- Next step: make temporary registration path-aware inside conditional expressions and flush only
  the arm that ran, after its value has been bound. Pin both selected-arm directions with JIT and
  native lifecycle counters.
