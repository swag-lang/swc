...
bool MicroLoadStoreForwardingPass::getStackSlotKey(StackSlotKey& outKey, const MicroInstr& inst, const MicroInstrOperand* ops) const
{
    ...
    if (!MicroPassHelpers::isStackBaseRegister(*context_, baseReg))
        return false;
...
void MicroLoadStoreForwardingPass::invalidateOverlappingSlots(StackSlotMap& slots, const StackSlotKey& targetKey)
{
...
void MicroLoadStoreForwardingPass::invalidateSlotsUsingRegister(StackSlotMap& slots, const MicroReg reg)
{
...
bool MicroLoadStoreForwardingPass::isSameMemoryAddress(const MicroInstrOperand* storeOps, const MicroInstrOperand* loadOps)
{
...
bool MicroLoadStoreForwardingPass::isSameMemoryAddressForImmediateStore(const MicroInstrOperand* storeOps, const MicroInstrOperand* loadOps)
{
    return storeOps[0].reg == loadOps[1].reg &&
...
bool MicroLoadStoreForwardingPass::canCrossInstruction(const MicroInstr& store, const MicroInstrOperand* storeOps, const MicroInstr& scanInst) const
{
    SWC_ASSERT(context_ != nullptr);
