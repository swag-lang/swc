...
### scope.midi.001 — MIDI has no playback or synthesis
...
### scope.midi.002 — MIDI transport has no loop, metronome, count-in, or playback rate
...
- Next: add a tempo-map-aware transport-range model after scope.midi.001, with musical and absolute time
  endpoints.
- Complete when: loop boundaries can snap to event/beat/bar or remain exact, metronome follows time
  ...
  agree.
- Related: scope.midi.001

### scope.midi.003 — The piano roll has no playhead, overview, or direct musical-time address
...
### scope.midi.004 — Notes have no selection or detail inspector
...
### scope.midi.005 — Non-note MIDI events cannot be viewed on an event list
...
### scope.midi.006 — Tempo, meter, key, marker, and lyric changes are reduced to first-value summary
...
### scope.midi.007 — Tracks and channels cannot be muted, soloed, regrouped, or understood
...
- Next: introduce a track/channel matrix with stable colors and diagnostic mute/solo state independent
  of source events.
- Complete when: track and channel filters compose, mute/solo affect display and scope.midi.001 playback,
...
### scope.midi.008 — Controller, pitch, velocity, aftertouch, and program automation have no lanes
...
### scope.midi.009 — The piano roll cannot zoom vertically or adapt its keyboard and note labels
...
### scope.midi.010 — SMF timing and structural edge cases have no explicit support matrix
...
### scope.midi.011 — Large MIDI files are fully materialized and hard-capped
...
### scope.midi.012 — MIDI events and views cannot be exported with an explicit loss policy
