...
namespace
{
    ...
    bool materializeDefaultConstantPayload(CodeGen& codeGen, CodeGenNodePayload& outPayload, TypeRef targetTypeRef, ConstantRef defaultCstRef)
    {
        ...
        {
            materializedCstRef = CodeGenFunctionHelpers::materializeStaticPayloadConstant(codeGen, storageTypeRef, ByteSpan{rawBytes.data(), rawBytes.size()});
        }
        else
        {
            ...
            materializedCstRef = codeGen.cstMgr().addConstant(ctx, materializedCst);
        }
...
ConstantRef CodeGenFunctionHelpers::materializeStaticPayloadConstant(CodeGen& codeGen, TypeRef typeRef, ByteSpan payload)
{
    if (typeRef.isInvalid())
        return ConstantRef::invalid();
