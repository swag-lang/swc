...
Diagnostic SemaError::reportError(Sema& sema, DiagnosticId id, AstNodeRef nodeRef)
{
...
Diagnostic SemaError::reportError(Sema& sema, DiagnosticId id, SourceViewRef srcViewRef, TokenRef tokRef)
{
    auto diag = Diagnostic::get(id, sema.ast().srcView().fileRef());
...
Diagnostic SemaError::reportError(Sema& sema, DiagnosticId id, SourceViewRef srcViewRef, TokenRef tokRef, AstNodeRef nodeSpanRef)
{
    auto                     diag = reportError(sema, id, srcViewRef, tokRef);
...
void SemaError::raiseError(Sema& sema, DiagnosticId id, SourceViewRef srcViewRef, TokenRef tokRef)
{
    const auto diag = reportError(sema, id, srcViewRef, tokRef);
    diag.report(sema.ctx());
}

void SemaError::raiseError(Sema& sema, DiagnosticId id, AstNodeRef nodeRef)
{
    const auto diag = reportError(sema, id, nodeRef);
    diag.report(sema.ctx());
}

void SemaError::raiseInvalidType(Sema& sema, AstNodeRef nodeRef, TypeRef srcTypeRef, TypeRef targetTypeRef)
{
    auto diag = reportError(sema, DiagnosticId::sema_err_invalid_type, nodeRef);
...
{
    auto diag = reportError(sema, DiagnosticId::sema_err_cannot_cast, nodeRef);
...
{
    auto diag = reportError(sema, DiagnosticId::sema_err_literal_overflow, nodeRef);
...
{
    auto diag = reportError(sema, DiagnosticId::sema_err_literal_too_big, nodeRef);
    diag.addArgument(Diagnostic::ARG_VALUE, literal.toString(sema.ctx()));
    diag.report(sema.ctx());
}
...
{
    auto diag = reportError(sema, DiagnosticId::sema_err_division_zero, nodeOp.srcViewRef(), nodeOp.tokRef(), nodeValueRef);
    diag.addArgument(Diagnostic::ARG_TYPE, targetTypeRef);
    diag.report(sema.ctx());
}
...
{
    return raiseError(sema, DiagnosticId::sema_err_expr_not_const, nodeRef);
}

void SemaError::raiseBinaryOperandType(Sema& sema, const AstNode& nodeOp, AstNodeRef nodeValueRef, TypeRef targetTypeRef)
{
    auto diag = reportError(sema, DiagnosticId::sema_err_binary_operand_type, nodeOp.srcViewRef(), nodeOp.tokRef(), nodeValueRef);
    diag.addArgument(Diagnostic::ARG_TYPE, targetTypeRef);
    diag.report(sema.ctx());
}

void SemaError::raiseInternalError(Sema& sema, const AstNode& node)
{
    raiseError(sema, DiagnosticId::sema_err_internal, node.srcViewRef(), node.tokRef());
}
