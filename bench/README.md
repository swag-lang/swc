...
```
tools\bench.swgs --label "what changed since last time"
```
...
| | |
|---|---|
| `tools\bench.swgs --quick` | one sample, no warm-up; proves the plumbing works and is **not** recorded |
| `tools\bench.swgs --report-only` | rebuild the normalized history and page from raw campaigns, measure nothing |
| `tools\bench.swgs --no-build` | measure the binary already in `bin/`, useful when iterating on the harness |
