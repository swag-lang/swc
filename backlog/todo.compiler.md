...
## Tier C — Packed vector (`#simd`) follow-through

`#simd [N] T` shipped as a 128-bit value type: VEX encodings, lane-legal operators, `@vec*`
intrinsics, Swag-callconv argument and return passing in one XMM register, and cross-module
exports. The entries below are the capabilities intentionally left out of that first cut.

### T-505 — A `#simd` argument cannot occupy a stack slot

**Intent.** Pass packed arguments beyond the four common register lanes. Call stack and home
slots are 8 bytes, so today a 128-bit argument that would land in a stack slot is guarded by
`SWC_INTERNAL_CHECK` in `ABICall` (both the prepared path and the JIT bridge), and pure-call
constant folding gives up in `SemaJIT::buildConstCallArguments` when a simd argument would
reach a stack lane. Choose one convention — 16-byte stack slots for packed arguments, or
indirect passing like large structs — and apply it to the caller, the callee prologue, and the
JIT bridge together.

**Complete when.** A Swag-callconv function whose fifth-or-later parameter is `#simd` runs
correctly in JIT and native code, pure-call folding no longer bails for it, and the
`ABICall` guards are removed.

### T-506 — 256-bit vectors are not expressible

**Intent.** Extend the type constructor to 32-byte geometry (`#simd [32] u8`, `#simd [8] f32`)
gated on AVX2, with a register class for YMM, spill slots and constants at 32 bytes, and the
same lane-legality table. The 128-bit design anticipated this: the shape is parsed from the
array form, so no new keywords are needed.

**Complete when.** A 32-byte `#simd` type compiles, runs, and round-trips a cross-module
export, and 128-bit code generation is byte-identical to before the extension.

### T-507 — Packed code generation misses idiomatic hardware forms

**Intent.** Emit the tighter encodings the first cut leaves on the table: constant `@vecperm`
masks as `pshufd`/`pshufb` with an immediate where the pattern allows, `@vecselect` as
`vpblendvb`, fused multiply-add contraction when the target ISA has FMA, `@vecsad` for
absolute-difference sums, horizontal reductions (`@vecany`/`@vecall` today movmsk-and-test,
sums via `phadd`/shuffle ladders), and lane extract/insert through `pextrN`/`pinsrN` instead
of a spill slot.

**Complete when.** Each form is covered by an encoder test and visible in `PrintMicro` output
on a representative `bin/std` kernel, with the H.264 half-pel and averaging kernels showing
