...
namespace
{
    ...
    void updateDefaultBuildCfg(CommandLine& cmdLine)
    {
        ...
        if (cmdLine.backendKindName == "exe" || cmdLine.backendKindName == "all")
    ...
    global_(&global)
{
    ...
    addArg(HelpOptionGroup::Target, "build run", "--backend-kind", nullptr, CommandLineType::EnumString, &cmdLine_->backendKindName, "exe|dll|lib|all", "Select the native artifact kind generated by the native backend.");
    ...
    addArg(HelpOptionGroup::Target, "build run", "--name", nullptr, CommandLineType::String, &cmdLine_->name, nullptr, "Set the artifact name.");
    addArg(HelpOptionGroup::Target, "build run", "--out-dir", nullptr, CommandLineType::Path, &cmdLine_->outDir, nullptr, "Set the artifact output directory.");
    addArg(HelpOptionGroup::Target, "build run", "--work-dir", nullptr, CommandLineType::Path, &cmdLine_->workDir, nullptr, "Set the work directory.");
    ...
    addArg(HelpOptionGroup::Compiler, "all", "--clear", nullptr, CommandLineType::Bool, &cmdLine_->clear, nullptr, "Clear native work and artifact folders before building.");
    ...
    addArg(HelpOptionGroup::Diagnostics, "all", "--verbose-verify", "-vv", CommandLineType::Bool, &cmdLine_->verboseVerify, nullptr, "Log diagnostics that are normally suppressed by --test.");
    ...
    addArg(HelpOptionGroup::Diagnostics, "all", "--test", "-t", CommandLineType::Bool, &cmdLine_->test, nullptr, "Enable source-driven test mode, expected diagnostics, and #test functions.");
