        ...
        if (srcType.isAggregate())
        {
            ...
            if (castCtx.isConstantFolding())
            {
                ...
                for (size_t i = 0; i < values.size(); ++i)
                {
                    ...
                    const Result res = Cast::castAllowed(sema, elemCtx, srcTypes[i], dstElemTypeRef);
                    if (res != Result::Continue)
                    {
                        castCtx.failure = elemCtx.failure;
                        return res;
        ...
        if (srcType.isAggregateStruct())
        {
            ...
            if (castCtx.isConstantFolding())
            {
                ...
                if (cst.isAggregateStruct())
                {
                    ...
                    const bool ok = ConstantHelpers::lowerAggregateStructToBytes(sema, ByteSpan{buffer.data(), buffer.size()}, dstType, newValues);
                    if (!ok)
                    {
                        castCtx.fail(DiagnosticId::sema_err_cannot_cast, srcTypeRef, dstTypeRef);
                        return Result::Error;
