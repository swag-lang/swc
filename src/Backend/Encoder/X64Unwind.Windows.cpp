...
namespace
{
    ...
    constexpr uint8_t K_UWOP_SAVE_XMM128 = 8;
...
void X64UnwindWindows::buildInfo(ByteArray& outUnwindInfo, const uint32_t codeSize) const
{
    ...
    for (const UnwindOp& op : unwindOps)
    {
        switch (op.kind)
        {
            ...
            case UnwindOpKind::SaveXmm128:
                unwindSlots.push_back(packUnwindCodeSlot(op.codeOffset, K_UWOP_SAVE_XMM128, op.reg));
                unwindSlots.push_back(static_cast<uint16_t>(op.stackSize / 16));
                break;
...
void X64UnwindWindows::onInstructionEncoded(const MicroInstr& inst, const MicroInstrOperand* ops, const uint32_t codeStartOffset, const uint32_t codeEndOffset)
{
    ...
    switch (inst.op)
    {
        ...
        case MicroInstrOpcode::LoadMemReg:
            didTrack = tryTrackSaveXmm128(ops, codeEndOffset);
            break;
...
bool X64UnwindWindows::tryTrackSaveXmm128(const MicroInstrOperand* ops, const uint32_t codeEndOffset)
{
    // A prologue save of a nonvolatile xmm register: `movaps [rsp + off], xmmN`
    // after the fixed allocation, so the recorded offset is final-rsp-relative
    // as UWOP_SAVE_XMM128 requires.
    if (!ops)
        return false;
    if (!unwindHasStackAllocation_)
        return false;
    if (!ops[0].reg.isInt() || ops[0].reg.index() != 4)
        return false;
    if (!ops[1].reg.isFloat())
        return false;
    const uint32_t xmmIndex = ops[1].reg.index();
    if (xmmIndex < 6 || xmmIndex > 15)
        return false;
    if (ops[2].opBits != MicroOpBits::B128)
        return false;
    const uint64_t offset = ops[3].valueU64;
    if ((offset % 16) != 0 || offset / 16 > std::numeric_limits<uint16_t>::max())
        return false;

    UnwindOp op;
    op.kind       = UnwindOpKind::SaveXmm128;
    op.codeOffset = static_cast<uint8_t>(codeEndOffset);
    op.reg        = static_cast<uint8_t>(xmmIndex);
    op.stackSize  = static_cast<uint32_t>(offset);
    unwindOps_.push_back(op);
    return true;
}
