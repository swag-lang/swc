...
- Area: bin/std
- Found while: validating a runtime allocator rewrite, where `tools/scripts.swgs smoke` failed
...
- Evidence: `%TEMP%/swag-sandbox` holds 4125 `run-<pid>` directories, 846 of them non-empty. Id
  reuse is not theoretical at that density: one `tools/scripts.swgs dm smoke` pass of 21 scripts
...
- Evidence: on an unchanged tree, `tools/std.swgs build` reports every module `up-to-date` in
  1.4 s, while `tools/std.swgs test` over the same sources costs more than two minutes. The
...
- Next step: check whether dropping `CommandKind::Test` from that exclusion is now safe, keeping
  ...
  compile, never skipping the execution. Measure a second `tools/tests.swgs` pass on an untouched
  tree before and after.
- Related: entry 2 of [todo.compiler.md](todo.compiler.md), which is the same problem one level
...
- Observation: the file discovery behind `swc format` drops any path holding a segment that starts
  with a dot, and it drops it without a word. `tools/format.swgs` run from such a checkout reports
