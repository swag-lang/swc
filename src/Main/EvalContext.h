#pragma once
#include "Main/CompilerContext.h"

SWC_BEGIN_NAMESPACE()

class SourceFile;

class EvalContext
{
    const CompilerContext& cmpContext;
    SourceFile*            sourceFile_ = nullptr;

public:
    explicit EvalContext(const CompilerContext& compContext) :
        cmpContext(compContext)
    {
    }

    const Global&      global() const { return cmpContext.global(); }
    const CommandLine& cmdLine() const { return cmpContext.cmdLine(); }
    SourceFile*        sourceFile() const { return sourceFile_; }
    void               setSourceFile(SourceFile* sourceFile) { sourceFile_ = sourceFile; }
};

SWC_END_NAMESPACE()
