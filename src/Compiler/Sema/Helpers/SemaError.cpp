...
Result SemaError::raiseExprNotConst(Sema& sema, AstNodeRef nodeRef)
{
    ...
    if (lowest.isValid())
    {
        const SourceCodeRange codeRange = sema.node(lowest).codeRangeWithChildren(sema.ctx(), sema.ast());
...
Result SemaError::raiseBinaryOperandType(Sema& sema, const AstNode& nodeOp, AstNodeRef nodeValueRef, TypeRef targetTypeRef)
{
    ...
    const SourceCodeRange codeRange = sema.node(nodeValueRef).codeRangeWithChildren(sema.ctx(), sema.ast());
