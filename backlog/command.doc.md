...
### B-178 — Code examples in documentation are rendered, never compiled
...
### B-179 — Search stops at the page it is printed in
...
### B-180 — There is no serializable documentation model
...
### B-255 — No public-API diff consumes the documentation model

Once B-180 publishes stable item identities and signatures, compare two serialized models and
report source-compatible additions, removals, and changes without scraping HTML.

- Related: B-180

### B-256 — No documentation-coverage report
...
- Next step: attribute each newly emitted function and its lowering CPU to the first JIT root and
  ...
  doc-mode fast path exists. Persisting sema and codegen across invocations remains B-168/B-258.
- Related: B-168, B-258
