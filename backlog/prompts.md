...
Pick the task with the worst ratio that you have not already exhausted, then:
  ...
  5. Validate correctness before believing any number: tools/tests.swgs dm, then
     tools/tests.swgs dm --all-cfg. A checksum mismatch in bench means you measured nothing.
  ...
  7. Record a full campaign only when you have a result worth keeping:
     tools\bench.swgs --label "what changed". That takes ~25 minutes; do not spend one per experiment.
...
RULES
  ...
  - Correctness first, always. tools/tests.swgs dm and --all-cfg must be green before any number is
    believed, and the Release sequence before anything is recorded. A pass that miscompiles under
    the JIT but passes unit tests is the known failure mode here - tools/scripts.swgs dm is what
...
For each check, in this order, and do not skip step 1:
  ...
  3. Sweep the whole tree for false positives, and mean the whole tree: tools/build.swgs,
     tools/std.swgs, tools/apps.swgs, tools/examples.swgs, tools/reference.swgs. The baseline is zero
     ...
     Sweep with 'test' as well, or just run tools/tests.swgs dm and read its first failure.
  ...
  6. tools/tests.swgs dm, then --all-cfg, then the Release sequence.
...
A round is not done until all four hold:
  ...
  1. tools/tests.swgs dm, then tools/tests.swgs dm --all-cfg, then Release build, then
     tools/tests.swgs. All green.
...
Targets, all on this machine, all re-measured before you start:
  ...
  - tools/web.swgs (the whole documentation site) and tools/format.swgs (every Swag workspace):
...
THE LOOP
  ...
  5. tools/tests.swgs dm, --all-cfg, Release sequence.
