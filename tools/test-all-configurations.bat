...
rem Runs the complete repository test suite in release, debug, and fast-debug.
...
for %%C in (release debug fast-debug) do (
    call "%TOOLS_DIR%test-all-workspaces.bat" %MODE_ARG% --build-cfg "%%C"%EXTRA_ARGS% || exit /b 1
)
