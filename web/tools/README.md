...
| Tool | Produces |
| --- | --- |
| `../../tools/web.swgs` | Everything: it cuts the brand assets, then runs `swc doc` over the `bin/std` and `bin/reference` workspaces |
...
`web.swgs` accepts the same `dm` first argument as the other compiler
tools.
