...
swc test --artifact-kind exe -d "%ROOT%\bin\tests\native" --out-dir "%EXE_OUTPUT%\!BUILD_CFG!" --work-dir "%EXE_WORKDIR%\!BUILD_CFG!" --build-cfg !BUILD_CFG! !EXTRA_ARGS!
if errorlevel 1 exit /b 1
...
swc test -m "%ROOT%\bin\reference\tests\language" --artifact-kind exe --out-dir "%REFERENCE_OUTPUT%\!BUILD_CFG!" --work-dir "%REFERENCE_WORKDIR%\!BUILD_CFG!" --build-cfg !BUILD_CFG! !EXTRA_ARGS!
if errorlevel 1 exit /b 1
