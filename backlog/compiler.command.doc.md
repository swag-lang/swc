...
The command produces a complete static site — API pages collected from the semantic model, guide
...
second file: a page of eight entries or more prints its own search index and the one inline script that
...
- One shared index loaded by every page is what rustdoc does, and it is exactly the second file
  ...
  - A no-hit fallback: each page also knows the workspace's module page names, so a query with no
