...
namespace
{
    ...
    bool appendTypeNodeSource(Sema& sema, AstNodeRef nodeRef, Utf8& out)
    {
        const SourceCodeRange codeRange = sema.node(nodeRef).codeRangeWithChildren(sema.ctx(), sema.ast());
        if (!codeRange.srcView || !codeRange.len)
            return false;

        out += Utf8{codeRange.srcView->codeView(codeRange.offset, codeRange.len)};
        return true;
    }

    bool renderFullTypeExprName(Sema& sema, AstNodeRef typeNodeRef, Utf8& out)
    {
        const TaskContext& ctx  = sema.ctx();
        const AstNode&     node = sema.node(typeNodeRef);
        ...
        switch (node.id())
        {
            case AstNodeId::QualifiedType:
            {
                ...
                if (qualified.hasFlag(AstQualifiedTypeFlagsE::Nullable))
                    out += "#null ";
                if (qualified.hasFlag(AstQualifiedTypeFlagsE::Const))
                    out += "const ";
                return renderFullTypeExprName(sema, qualified.nodeTypeRef, out);
            }

            case AstNodeId::ValuePointerType:
            {
                out += "*";
                return renderFullTypeExprName(sema, node.cast<AstValuePointerType>().nodePointeeTypeRef, out);
            }

            case AstNodeId::BlockPointerType:
            {
                out += "[*] ";
                return renderFullTypeExprName(sema, node.cast<AstBlockPointerType>().nodePointeeTypeRef, out);
            }

            case AstNodeId::ReferenceType:
            {
                out += "&";
                return renderFullTypeExprName(sema, node.cast<AstReferenceType>().nodePointeeTypeRef, out);
            }

            case AstNodeId::MoveRefType:
            {
                out += "&&";
                return renderFullTypeExprName(sema, node.cast<AstMoveRefType>().nodePointeeTypeRef, out);
            }

            case AstNodeId::SliceType:
            {
                out += "[..] ";
                return renderFullTypeExprName(sema, node.cast<AstSliceType>().nodePointeeTypeRef, out);
            }

            case AstNodeId::ArrayType:
            {
                const auto& arrayType = node.cast<AstArrayType>();
                if (arrayType.spanDimensionsRef.isInvalid())
                {
                    out += "[?]";
                }
                else
                {
                    ...
                    for (size_t i = 0; i < dims.size(); ++i)
                    {
                        ...
                        const SemaNodeView dimView = sema.viewConstant(dims[i]);
                        if (dimView.cstRef().isValid())
                        {
                            out += dimView.cst()->toString(ctx);
                        }
                        else if (!appendTypeNodeSource(sema, dims[i], out))
                        {
                            return false;
                        }
                    }
                ...
                Utf8 elemName;
                if (!renderFullTypeExprName(sema, arrayType.nodePointeeTypeRef, elemName))
                    return false;
                if (!elemName.empty() && elemName[0] != '[')
                    out += " ";
                out += elemName;
                return true;
            }
            ...
            {
                if (!renderFullTypeExprName(sema, node.cast<AstTypedVariadicType>().nodeTypeRef, out))
                    return false;
                out += "...";
                return true;
            }
            ...
            {
                out += "#code ";
                return renderFullTypeExprName(sema, node.cast<AstCodeType>().nodeTypeRef, out);
            }

            case AstNodeId::LambdaType:
            {
                const auto& lambdaType = node.cast<AstLambdaType>();
                out += lambdaType.hasFlag(AstFunctionFlagsE::Method) ? "mtd" : "func";
                if (lambdaType.hasFlag(AstFunctionFlagsE::Closure))
                    out += "||";
                out += "(";
                ...
                for (size_t i = 0; i < params.size(); ++i)
                {
                    ...
                    const auto& param = sema.node(params[i]).cast<AstLambdaParam>();
                    if (param.hasFlag(AstLambdaParamFlagsE::Named))
                    {
                        const IdentifierRef idRef = sema.idMgr().addIdentifier(ctx, param.codeRef());
                        out += sema.idMgr().get(idRef).name;
                        out += ": ";
                    }

                    if (param.nodeTypeRef.isValid())
                    {
                        if (!renderFullTypeExprName(sema, param.nodeTypeRef, out))
                            return false;
                    }
                    else
                    {
                ...
                {
                    out += "->";
                    if (!renderFullTypeExprName(sema, lambdaType.nodeReturnTypeRef, out))
                        return false;
                }

                if (lambdaType.hasFlag(AstFunctionFlagsE::Throwable))
                    out += " throw";
                return true;
            }
            ...
            {
                Utf8 rendered;
                if (!appendTypeNodeSource(sema, typeNodeRef, rendered))
                    return false;
            ...
            default:
            {
                const SemaNodeView typeView = sema.viewType(typeNodeRef);
                if (!typeView.typeRef().isValid())
                    return false;
                out += sema.typeMgr().get(typeView.typeRef()).toFullName(ctx);
                return true;
            }
        }
    }
    ...
    Result semaCompilerFullNameOf(Sema& sema, const AstCompilerCallOne& node)
    {
        ...
        if (const auto* compilerTypeExpr = sema.node(childRef).safeCast<AstCompilerTypeExpr>())
        {
            Utf8 renderedName;
            if (renderFullTypeExprName(sema, compilerTypeExpr->nodeTypeRef, renderedName))
            {
                const ConstantValue value = ConstantValue::makeString(ctx, renderedName);
                sema.setConstant(sema.curNodeRef(), sema.cstMgr().addConstant(ctx, value));
                return Result::Continue;
            }
        }
...
Result AstCompilerFunc::semaPreDecl(Sema& sema)
{
    ...
    const bool   ignoreMainFunc     = tok.id == TokenId::CompilerFuncMain && ctx.cmdLine().backendKind != Runtime::BuildCfgBackendKind::Executable;
