...
- Done so far: the backend's SSE2 auto-vectorizer (release builds, `cpuVectorize`) compiles the
  ...
  frame on every round iteration, which is the store-forwarding chain F-026 in `FINDINGS.md`
  documents, with the re-measurement protocol to use once that lands.
- Fix: keep the four state rows register-resident across the ten rounds (backend work, F-026),
