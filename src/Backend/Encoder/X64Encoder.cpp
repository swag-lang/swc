...
void X64Encoder::onInstructionEncoded(const MicroInstr& inst, const MicroInstrOperand* ops, const uint32_t codeStartOffset, const uint32_t codeEndOffset)
{
    ...
    switch (inst.op)
    {
        case MicroInstrOpcode::Push:
            didTrack = tryTrackUnwindPush(ops, codeEndOffset);
            break;

        case MicroInstrOpcode::LoadRegReg:
        case MicroInstrOpcode::LoadAddrRegMem:
            didTrack = tryTrackUnwindSetFramePointer(inst, ops, codeEndOffset);
            break;

        case MicroInstrOpcode::LoadMemReg:
            didTrack = tryTrackUnwindSaveNonVol(ops, codeEndOffset);
            break;

        case MicroInstrOpcode::OpBinaryRegImm:
            didTrack = tryTrackUnwindAllocateStack(ops, codeEndOffset);
            break;
...
bool X64Encoder::tryTrackUnwindPush(const MicroInstrOperand* ops, const uint32_t codeEndOffset)
{
    if (!ops)
        return false;

    uint8_t reg = 0;
    if (!tryMapWindowsUnwindReg(reg, ops[0].reg))
        return false;
    if (!isWindowsX64NonVolatileReg(reg))
        return false;
...
bool X64Encoder::tryTrackUnwindAllocateStack(const MicroInstrOperand* ops, const uint32_t codeEndOffset)
{
    if (!ops)
        return false;
...
bool X64Encoder::tryTrackUnwindSetFramePointer(const MicroInstr& inst, const MicroInstrOperand* ops, const uint32_t codeEndOffset)
{
    if (!ops)
        return false;
    ...
    uint8_t reg = 0;
    if (!tryMapWindowsUnwindReg(reg, frameReg))
        return false;
    if (!isWindowsX64NonVolatileReg(reg))
        return false;
...
bool X64Encoder::tryTrackUnwindSaveNonVol(const MicroInstrOperand* ops, const uint32_t codeEndOffset)
{
    if (!ops)
        return false;
    ...
    uint8_t reg = 0;
    if (!tryMapWindowsUnwindReg(reg, ops[1].reg))
        return false;
    if (!isWindowsX64NonVolatileReg(reg))
        return false;
...
void X64Encoder::closeUnwindProlog()
{
    unwindPrologClosed_ = true;
}

bool X64Encoder::canTrackUnwindInstruction(const uint32_t codeEndOffset)
{
