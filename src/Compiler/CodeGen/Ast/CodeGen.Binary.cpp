...
namespace
{
    ...
    Result emitPlusIntLikeIntLike(CodeGen& codeGen, const AstBinaryExpr& node, const SemaNodeView& leftView, const SemaNodeView& rightView)
    {
        ...
        CodeGenNodePayload& nodePayload = codeGen.setPayloadValue(codeGen.curNodeRef(), codeGen.sema().nodeViewType(codeGen.curNodeRef()).typeRef());
    ...
    {
        const SemaNodeView leftView  = codeGen.sema().nodeViewType(node.nodeLeftRef);
        const SemaNodeView rightView = codeGen.sema().nodeViewType(node.nodeRightRef);
        SWC_ASSERT(leftView.type() && rightView.type());
