...
### scope.image.001 — The image has no pixel probe or measurement tools
...
### scope.image.002 — Image analysis has no histogram, channel, clipping, or transparency views
...
### scope.image.003 — Color management and HDR state are invisible to the reader

- Evidence: pixel.image.018/pixel.image.019 record that the pixel stack has no colour/ICC handling, and the image
...
- Related: pixel.image.018, pixel.image.019, pixel.image.020, pixel.image.021

### scope.image.004 — Source orientation is not distinguished from the temporary view transform
...
- Next: expose normalized source orientation through scope.image.011, then define one coordinate mapping for
  image dimensions, pan, selection, probes, animation frames, and the products in scope.image.009.
...
### scope.image.006 — Images cannot be compared side by side, overlaid, or by difference
...
### scope.image.007 — Animated images have no frame-step, speed, loop, or disposal inspection
...
### scope.image.008 — Multi-image and multi-page formats have no collection model
...
### scope.image.010 — Huge and damaged images cannot degrade progressively
...
### scope.image.011 — An image's metadata is not shown
...
- Related: pixel.image.018, pixel.image.019

### scope.image.012 — Camera RAW files show nothing
...
- Complete when: the embedded preview of the common TIFF-based RAW containers is extracted and
  displayed, with the metadata panel from scope.image.011 beside it.
- Related: scope.image.011
