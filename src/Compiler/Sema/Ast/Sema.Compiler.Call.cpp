    ...
    Result substituteCompilerInject(Sema& sema, AstNodeRef ownerRef, AstNodeRef exprRef, std::span<const SemaClone::NodeReplacement> replacements = std::span<const SemaClone::NodeReplacement>{})
    {
        ...
        if (isMacroInject)
        {
            ...
            if (injectsVoidCode)
            {
                ...
                inlineOverridePayload->targetInlinePayload = SemaInline::returnContextPayload(inlinePayload->parentInlinePayload);
...
namespace
{
    ...
    Result semaCompilerRunes(Sema& sema, const AstCompilerCallOne& node)
    {
        ...
        const std::span     bytes = {reinterpret_cast<const std::byte*>(runes.data()), runes.size() * sizeof(char32_t)};
...
Result AstCompilerCallOne::semaPostNode(Sema& sema) const
{
    ...
    switch (tok.id)
    {
        ...
        case TokenId::CompilerSanity:
            return semaCompilerSanity(sema, *this);
