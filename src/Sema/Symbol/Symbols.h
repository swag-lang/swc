...
public:
    ...
    const TypeInfo& underlyingType(TaskContext& ctx) const;
    ...
    bool          assignExplicitValue(Sema& sema, const ConstantValue& val);
    bool          computeNextAutoValue(Sema& sema, SourceViewRef srcViewRef, TokenRef tokRef);
