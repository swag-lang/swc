...
void Sema::raiseInvalidType(AstNodeRef nodeRef, TypeRef srcTypeRef, TypeRef targetTypeRef) const
{
    ...
    diag.addArgument(Diagnostic::ARG_TYPE, typeMgr().typeToName(srcTypeRef));
    diag.addArgument(Diagnostic::ARG_REQUESTED_TYPE, typeMgr().typeToName(targetTypeRef));
    diag.report(*ctx_);
}
...
void Sema::raiseLiteralTooBig(AstNodeRef nodeRef, const ConstantValue& literal) const
{
    auto diag = reportError(DiagnosticId::sema_err_literal_too_big, nodeRef);
    diag.addArgument(Diagnostic::ARG_VALUE, literal.toString(*ctx_));
    diag.report(*ctx_);
}
