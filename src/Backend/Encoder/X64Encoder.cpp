...
bool X64Encoder::queryConformanceIssue(MicroConformanceIssue& outIssue, const MicroInstr& inst, const MicroInstrOperand* ops) const
{
    ...
    if (inst.op == MicroInstrOpcode::OpBinaryRegReg)
    {
        ...
        if ((op == MicroOp::MultiplyHighUnsigned || op == MicroOp::MultiplyHighSigned) &&
            (ops[2].opBits == MicroOpBits::B32 || ops[2].opBits == MicroOpBits::B64))
        {
            // Expose MUL/IMUL's high output to allocation instead of hiding an RDX-to-RAX
            ...
            outIssue.replacementOp = op == MicroOp::MultiplyHighSigned ? MicroOp::MultiplyWideSigned : MicroOp::MultiplyUnsigned;
            return true;
        }

        const bool isB8SignedMul = op == MicroOp::MultiplySigned && ops[2].opBits == MicroOpBits::B8;
        if (op == MicroOp::MultiplyUnsigned || op == MicroOp::MultiplyWideSigned ||
            isB8SignedMul ||
            op == MicroOp::MultiplyHighSigned ||
            op == MicroOp::MultiplyHighUnsigned ||
    ...
    ///////////////////////////////////////////
    if ((inst.op == MicroInstrOpcode::TestRegReg || inst.op == MicroInstrOpcode::TestMemReg) && requireStandardIntOpBits(outIssue, ops[2].opBits, 2))
        return true;

    // Tests and compares share the immediate encoding limits. The memory
    // compare spends an extra operand on its address.
    if (inst.op == MicroInstrOpcode::TestRegImm || inst.op == MicroInstrOpcode::TestMemImm || inst.op == MicroInstrOpcode::CmpRegImm || inst.op == MicroInstrOpcode::CmpMemImm)
    {
        if (requireStandardIntOpBits(outIssue, ops[1].opBits, 1))
            return true;

        const uint32_t immediateIndex = (inst.op == MicroInstrOpcode::CmpMemImm || inst.op == MicroInstrOpcode::TestMemImm) ? 3 : 2;
...
void X64Encoder::encodeAddCarryRegImm(MicroReg regDst, uint64_t value, MicroOpBits bits)
{
    ...
    emitModRm(store_, MODRM_REG_2, regDst);
...
void X64Encoder::encodeSubtractBorrowRegReg(MicroReg regDst, MicroReg regSrc, MicroOpBits bits)
{
    ...
    emitRex(store_, bits, regSrc, regDst);
    emitCpuOp(store_, 0x19);
    emitModRm(store_, regSrc, regDst);
}
...
namespace
{
    ...
    void encodeAmcOpImm(PagedStore& store, MicroReg regBase, MicroReg regMul, uint64_t mulValue, uint64_t addValue, const ApInt& value, MicroOp op, MicroOpBits opBitsValue)
    {
        ...
        // ModRM
        if (needsForcedDisplacement)
            emitModRm(store, canEncodeSigned8(addValue) ? ModRmMode::Displacement8 : ModRmMode::Displacement32, group, MODRM_RM_SIB);
        else if (addValue == 0 || baseIsNoBase)
            emitModRm(store, ModRmMode::Memory, group, MODRM_RM_SIB);
        else
            emitModRm(store, canEncodeSigned8(addValue) ? ModRmMode::Displacement8 : ModRmMode::Displacement32, group, MODRM_RM_SIB);
...
void X64Encoder::encodeOpBinaryAmcMemImm(MicroReg regBase, MicroReg regMul, uint64_t mulValue, uint64_t addValue, const ApInt& value, MicroOp op, MicroOpBits opBits)
{
    SWC_ASSERT(regBase.isInt() && regMul.isInt());
    return encodeAmcOpImm(store_, regBase, regMul, mulValue, addValue, value, op, opBits);
}
...
void X64Encoder::encodeOpBinaryRegReg(MicroReg regDst, MicroReg regSrc, MicroOp op, MicroOpBits opBits)
{
    ...
    else if (op == MicroOp::MultiplyUnsigned || op == MicroOp::MultiplyWideSigned)
    {
        SWC_ASSERT(op != MicroOp::MultiplyWideSigned || opBits == MicroOpBits::B32 || opBits == MicroOpBits::B64);
        const auto rax = x64RegToMicroReg(X64Reg::Rax);
        emitRex(store_, opBits, rax, regSrc);
        emitSpecCpuOp(store_, MicroOp::BitwiseNot, opBits);
        emitModRm(store_, op == MicroOp::MultiplyWideSigned ? MODRM_REG_5 : MODRM_REG_4, regSrc);
    }
...
void X64Encoder::encodeOpBinaryRegImm(MicroReg reg, const ApInt& valueInt, MicroOp op, MicroOpBits opBits)
{
    ...
    ///////////////////////////////////////////
             ...
             op == MicroOp::MultiplyHighUnsigned)
    {
        SWC_ASSERT(!(op == MicroOp::ModuloUnsigned || op == MicroOp::ModuloSigned || op == MicroOp::DivideUnsigned || op == MicroOp::DivideSigned || op == MicroOp::MultiplyUnsigned || op == MicroOp::MultiplyWideSigned || op == MicroOp::MultiplyHighSigned || op == MicroOp::MultiplyHighUnsigned));
    }
...
void X64Encoder::encodeOpBinaryMemImm(MicroReg memReg, uint64_t memOffset, const ApInt& valueInt, MicroOp op, MicroOpBits opBits)
{
    ...
    // A global arithmetic or bitwise operation can update its segment slot
    // directly. The immediate follows the RIP displacement in each form.
    const auto emitImmediateMemoryOperand = [&](const uint8_t regField) {
        if (memReg.isInstructionPointer())
        {
            SWC_ASSERT(memOffset == 0 && (op == MicroOp::Add || op == MicroOp::Subtract ||
                                          op == MicroOp::And || op == MicroOp::Or || op == MicroOp::Xor));
            emitModRm(store_, ModRmMode::Memory, regField, MODRM_RM_RIP);
            store_.pushU32(0);
        }
        else
    ...
    else if (op == MicroOp::Add)
    {
        if (opBits == MicroOpBits::B8)
        {
            emitRex(store_, opBits, MicroReg{}, memReg);
            emitCpuOp(store_, 0x80);
            emitImmediateMemoryOperand(MODRM_REG_0);
            emitValue(store_, value, MicroOpBits::B8);
        }
        else if (canEncode8(value, opBits))
        {
    ...
    else if (op == MicroOp::Subtract)
    {
        if (opBits == MicroOpBits::B8)
        {
            emitRex(store_, opBits, MicroReg{}, memReg);
            emitCpuOp(store_, 0x80);
            emitImmediateMemoryOperand(MODRM_REG_5);
            emitValue(store_, value, MicroOpBits::B8);
        }
        else if (canEncode8(value, opBits))
        {
    ...
    else if (op == MicroOp::Or)
    {
        if (opBits == MicroOpBits::B8)
        {
            emitRex(store_, opBits, MicroReg{}, memReg);
            emitCpuOp(store_, 0x80);
            emitImmediateMemoryOperand(MODRM_REG_1);
            emitValue(store_, value, MicroOpBits::B8);
        }
        else if (canEncode8(value, opBits))
        {
    ...
    else if (op == MicroOp::And)
    {
        if (opBits == MicroOpBits::B8)
        {
            emitRex(store_, opBits, MicroReg{}, memReg);
            emitCpuOp(store_, 0x80);
            emitImmediateMemoryOperand(MODRM_REG_4);
            emitValue(store_, value, MicroOpBits::B8);
        }
        else if (canEncode8(value, opBits))
        {
    ...
    else if (op == MicroOp::Xor)
    {
        if (opBits == MicroOpBits::B8)
        {
            emitRex(store_, opBits, MicroReg{}, memReg);
            emitCpuOp(store_, 0x80);
            emitImmediateMemoryOperand(MODRM_REG_6);
            emitValue(store_, value, MicroOpBits::B8);
        }
        else if (canEncode8(value, opBits))
        {
