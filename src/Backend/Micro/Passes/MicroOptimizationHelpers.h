...
{
    inline bool isIdentityBinaryImm(MicroOp op, uint64_t value, MicroOpBits opBits)
    {
        switch (op)
        {
            case MicroOp::Add:
    ...
    inline bool isNoOpLoadRegReg(const MicroInstr& inst, const MicroInstrOperand* ops)
    {
        return inst.numOperands >= 2 && ops[0].reg == ops[1].reg;
    }

    inline bool isNoOpLoadAddrRegMem(const MicroInstr& inst, const MicroInstrOperand* ops)
    {
    ...
    inline bool isNoOpLoadCondRegReg(const MicroInstr& inst, const MicroInstrOperand* ops)
    {
        return inst.numOperands >= 4 && ops[3].opBits == MicroOpBits::B64 && ops[0].reg == ops[1].reg;
    }

    inline bool isNoOpOpBinaryRegReg(const MicroInstr& inst, const MicroInstrOperand* ops)
    {
        return ops[3].microOp == MicroOp::Exchange && ops[0].reg == ops[1].reg;
    }

    inline bool isNoOpOpBinaryRegImm(const MicroInstr& inst, const MicroInstrOperand* ops)
    {
        return isIdentityBinaryImm(ops[2].microOp, ops[3].valueU64, ops[1].opBits);
    }

    inline bool isNoOpEncoderInstruction(const MicroInstr& inst, const MicroInstrOperand* ops)
    {
