...
default `loadDatas = true`, it also keeps the audio payload in memory. Pass
`loadDatas = false` for long sounds that should be read from disk while they
play.

```swag
let music = try Audio.SoundFile.load("assets/music.wav", loadDatas = false)
...
[[Audio.VoiceCreateFlags.AcceptPitch]] on backends that need that capability
