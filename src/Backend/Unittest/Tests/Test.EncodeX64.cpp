...
#define ENCODE_CASE(__name, __hex, ...)                                        \
    ...
    Result buildFlow(const RunCaseFn& runCase)
    {
        ...
        ENCODE_CASE("call_local", "48 B8 00 00 00 00 00 00 00 00 FF D0", b.encodeCallLocal({}, CallConvKind::C, K_EMIT););
        ENCODE_CASE("call_extern", "48 B8 00 00 00 00 00 00 00 00 FF D0", b.encodeCallExtern(nullptr, CallConvKind::C, K_EMIT););
        ENCODE_CASE("call_local_repeat", "48 B8 00 00 00 00 00 00 00 00 FF D0", b.encodeCallLocal({}, CallConvKind::C, K_EMIT););
        ...
        ENCODE_CASE("jump_not_zero_b8_patch_here", "75 00",
                    ...
                    b.encodeJumpToLabel(MicroCond::NotZero, MicroOpBits::B8, l, K_EMIT);
        ...
        ENCODE_CASE("jump_less_b32_patch_here", "0F 8C 00 00 00 00",
                    const auto l = b.createLabel();
                    b.encodeJumpToLabel(MicroCond::Less, MicroOpBits::B32, l, K_EMIT);
                    b.placeLabel(l, K_EMIT););
    ...
    Result buildLoad(const RunCaseFn& runCase)
    {
        ...
        ENCODE_CASE("load_reg_imm_xmm0_b32_conform", "48 83 EC 08 40 C7 04 24 01 00 00 00 F3 40 0F 10 04 24 48 83 C4 08", b.encodeLoadRegImm(XMM0, 1, MicroOpBits::B32, K_EMIT););
