...
Result AstMemberAccessExpr::semaPreNodeChild(Sema& sema, const AstNodeRef& childRef)
{
    ...
    const SemaNodeView nodeRightView = sema.nodeView(nodeRightRef);
    ...
    else if (typeInfo->isTypeInfo())
    {
        ...
        RESULT_VERIFY(sema.waitPredefined(IdentifierManager::PredefinedName::TypeInfo, typeInfoRef, {srcViewRef(), tokNameRef}));
