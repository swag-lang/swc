...
struct JobManager::RecordPool {
    ...
    static thread_local std::vector<Record*> tls;

    // Global fallback (small), guarded by a mutex.
    static std::mutex gMutex;
    static std::vector<Record*> gFree;
    static constexpr std::size_t kTLSMax = 1024; // cap per thread to avoid unbounded growth
};

thread_local std::vector<JobManager::Record*> JobManager::RecordPool::tls;
std::mutex JobManager::RecordPool::gMutex;
std::vector<JobManager::Record*> JobManager::RecordPool::gFree;

JobManager::Record* JobManager::allocRecord() {
    // Fast path: thread-local
    auto& v = RecordPool::tls;
    if (!v.empty()) {
        Record* r = v.back(); v.pop_back();
        return r;
    }
    // Slow path: global pool
    std::lock_guard<std::mutex> lk(RecordPool::gMutex);
    if (!RecordPool::gFree.empty()) {
        Record* r = RecordPool::gFree.back(); RecordPool::gFree.pop_back();
        return r;
    }
    // Fallback: heap
    return new Record();
...
void JobManager::freeRecord(Record* r) {
    if (!r) return;
    ...
    r->state = Record::State::Ready;
    r->prio  = Priority::Normal;

    // Try to return to TLS; spill to global if TLS is full.
    auto& v = RecordPool::tls;
    if (v.size() < RecordPool::kTLSMax) {
        v.push_back(r);
        return;
    }
    std::lock_guard<std::mutex> lk(RecordPool::gMutex);
...
bool JobManager::enqueue(const JobRef& job, Priority prio) {
    if (!job) return false;
...
bool JobManager::wake(const JobRef& job) {
    ...
    if (job->owner_ != this || job->rec_ == nullptr) return false;

    Record* rec = job->rec_;
    // Arm against lost-wake during a run.
    ++rec->wakeGen;

    if (rec->state == Record::State::Waiting) {
        rec->state = Record::State::Ready;
...
void JobManager::pushReady(Record* rec, Priority prio) {
...
bool JobManager::linkOrSkip(Record* waiter, Record* dep) {
    if (!dep || dep->state == Record::State::Done)
        return false;
    dep->dependents.push_back(waiter->job.get()); // store Job* for the waiter
    waiter->state = Record::State::Waiting;
    return true;
}

// Wake everyone waiting on 'finished'.
// We copy out the vector (move) to minimize lock hold time and avoid re-entrancy pitfalls.
void JobManager::notifyDependents(Record* finished) {
    auto deps = std::move(finished->dependents);
    ...
    for (Job* wjob : deps) {
        ...
        if (wjob && wjob->owner_ == this && wjob->rec_) {
            Record* wrec = wjob->rec_;
            ++wrec->wakeGen; // make future Sleep a no-op (requeue)
            if (wrec->state == Record::State::Waiting) {
                wrec->state = Record::State::Ready;
                pushReady(wrec, wrec->prio);
            }
        }
    }
    if (!deps.empty()) cv_.notify_all();
}

// Favor High → Normal → Low. Caller must hold mtx_.
JobManager::Record* JobManager::popReadyLocked() {
    for (int i = 0; i < 3; ++i) {
        auto& q = readyQ_[i];
        if (!q.empty()) {
            Record* rec = q.front();
...
void JobManager::workerLoop() {
    ...
    for (;;) {
        ...
        while (readyCount_.load(std::memory_order_acquire) == 0 && accepting_) {
            ...
            if (!accepting_ && readyCount_.load(std::memory_order_acquire) == 0) return;
            rec = popReadyLocked();
            if (rec) {
                if (rec->state == Record::State::Done) { rec = nullptr; continue; }
                rec->state = Record::State::Running;
                ++activeWorkers_;
            }
            goto have_job;
        }
...
have_job:
        ...
        Job::Result res;
        try { res = rec->job->process(); }
        catch (...) { res = Job::Result::Done; } // keep the pool alive

        {
            std::unique_lock<std::mutex> lk(mtx_);

            auto finish = [&](Record* r){
                r->state = Record::State::Done;
...
void JobManager::Job::wakeDependents() {
    if (!owner_ || !rec_) return; // not scheduled
