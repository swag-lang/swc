...
namespace
{
    void runExactCases(TaskContext& ctx)
    {
        ...
        const auto runCase = [&](const char* name, const char* expectedHex, const std::function<void(MicroInstrBuilder&)>& fn) {
    ...
    void runCoverageCases(TaskContext& ctx)
    {
        const auto runCase = [&](const char* name, const char* expectedHex, const std::function<void(MicroInstrBuilder&)>& fn) {
            X64Encoder encoder(ctx);
            Backend::Unittest::runEncodeCase(ctx, encoder, name, expectedHex, fn);
        ...
        runCase("sym_reloc_value_r8_b64_abs", "49 B8 30 00 00 00 00 00 00 00", [&](MicroInstrBuilder& builder) { builder.encodeLoadSymRelocValue(r8, 3, 0x30, MicroOpBits::B64, kEmitB64); });
