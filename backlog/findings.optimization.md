...
- Observation: rewriting an adjacent `mov %d, %a` / `%d op= %b` into `%d = %a op %b` in the
  ...
  for a while.
- Evidence: `tools/scripts.swgs dm` fails on every script, right after "tuned"; the failure is an
...
- Next step: two suspects remain. LICM treats an adjacent copy plus two-address compute as a pair it
  ...
  cheaply first: restrict the fold to `FloatAdd` alone and run `tools/scripts.swgs dm` - a crash
