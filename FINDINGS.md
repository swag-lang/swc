...
### F-035 — Removing duplicated C++ bodies barely shrinks the Release executable
...
- Observation: the Release link already runs `/OPT:ICF` (`EnableCOMDATFolding`) on top of LTCG, so
  ...
  binary.
- Evidence: 1398 net source lines removed across 85 files (verbatim twins such as `builtinTypeRef`,
  `canReflectTypeRef`, `emitCStringCountReg`, `applyAction`, `collectLoopBody`, plus the token
  classification moved into `Tokens.Def.inc`) moved `bin/swc.exe` from 5 082 624 to 5 072 384
  bytes — 10 240 bytes, 0.2%. Compile time and peak memory were unchanged: identical CPU median
  over twelve order-alternated rounds. Of that small gain, 1 536 bytes came not from deleting
  anything but from putting two helpers back inline in their headers after sharing them had cost
  them their inlining. The Release configuration already carries `/O2`, `/GL`, `/Gy`, `/OPT:REF`,
  `/OPT:ICF` and `RuntimeTypeInfo=false`, so the usual size switches are all on.
