...
namespace
{
    ...
    bool emitDefaultValueToLocalStack(CodeGen& codeGen, const SymbolVariable& symVar, const MicroReg dstReg, uint32_t localSize)
    {
        ...
        if (CodeGenMemoryHelpers::emitZeroOrSparsePayloadBytes(codeGen, dstReg, rawBytes, true))
            return true;
...
Result AstIdentifier::codeGenPostNode(CodeGen& codeGen)
{
    ...
    // read that was not cleared by an assignment target, address-of or null comparison). The
