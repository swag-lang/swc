...
The source material specific to generated website assets lives here. Project-owned entry
points are centralized in the repository-level `tools/` directory.

| Tool | Produces |
| --- | --- |
| `../../../tools/help.swgs` | Everything: it cuts the brand assets, then runs `swc doc` over the `bin/std` and `bin/reference` workspaces |
...
`help.swgs` accepts the same `dm` first argument as the other compiler
tools.
