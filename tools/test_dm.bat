...
set "REFERENCE_OUTPUT=%NATIVE_OUTPUT%\reference\devmode"
set "REFERENCE_WORKDIR=%NATIVE_OUTPUT%\work\reference\devmode"
...
for %%S in (lexer parser errors\lexer errors\parser sema errors\sema jit safety native) do (
    ...
    swc_devmode test --artifact-kind exe -d "%ROOT%\bin\tests\%%S" --module-namespace !MODULE_NAMESPACE! --out-dir "%EXE_OUTPUT%" --work-dir "%EXE_WORKDIR%" !STAGE_ARGS! %*
    if errorlevel 1 exit /b 1
)

swc_devmode test -m "%ROOT%\bin\reference\tests\language" --artifact-kind exe --module-namespace Language --out-dir "%REFERENCE_OUTPUT%" --work-dir "%REFERENCE_WORKDIR%" %*
if errorlevel 1 exit /b 1
