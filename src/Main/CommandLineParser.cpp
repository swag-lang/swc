...
bool CommandLineParser::getNextValue(const Context& ctx, const Utf8& arg, int& index, int argc, char* argv[], Utf8& value)
{
    if (index + 1 >= argc)
    {
        auto diag = Diagnostic::get(DiagnosticId::CmdLineMissingArgValue);
        setReportArguments(diag, arg);
        diag.report(ctx);
        return false;
    }
...
{
    auto diag = Diagnostic::get(DiagnosticId::CmdLineInvalidEnumValue);
...
std::optional<ArgInfo> CommandLineParser::findNegatedArgument(const Context& ctx, const Utf8& arg, const char* prefix, size_t noPrefixLen, const std::map<Utf8, ArgInfo>& argMap, bool& invertBoolean)
{
    ...
    {
        auto diag = Diagnostic::get(DiagnosticId::CmdLineInvalidBoolArg);
        setReportArguments(diag, info, arg);
        diag.report(ctx);
        return std::nullopt;
    }
...
{
    auto diag = Diagnostic::get(DiagnosticId::CmdLineInvalidArg);
    setReportArguments(diag, arg);
    diag.report(ctx);
}
...
Result CommandLineParser::parse(int argc, char* argv[])
{
    ...
    if (argc <= 1 || argv[1][0] == '-')
    {
        ...
        const auto diag = Diagnostic::get(DiagnosticId::CmdLineMissingCommand);
        diag.report(ctx);
        return Result::Error;
    }
    ...
    // Validate and set the command
    {
        ...
        {
            auto diag = Diagnostic::get(DiagnosticId::CmdLineInvalidCommand);
    ...
    for (int i = 2; i < argc; i++)
    {
        ...
        {
            auto diag = Diagnostic::get(DiagnosticId::CmdLineInvalidArgForCmd);
