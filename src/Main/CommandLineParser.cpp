...
Result CommandLineParser::checkCommandLine(TaskContext& ctx) const
{
    ...
    {
        fs::path temp = folder;
        SWC_RESULT_VERIFY(FileSystem::resolveFolder(ctx, temp));
    ...
    {
        fs::path temp = file;
        SWC_RESULT_VERIFY(FileSystem::resolveFile(ctx, temp));
    ...
    if (!cmdLine_->nativeArtifactOutputDir.empty())
    {
        std::error_code ec;
        fs::path        temp = fs::absolute(cmdLine_->nativeArtifactOutputDir, ec);
        if (ec)
        {
    ...
    global_(&global)
{
    ...
    addArg(HelpOptionGroup::Target, "test", "--backend-kind", nullptr, CommandLineType::EnumString, &cmdLine_->backendKindName, "exe|dll|lib|all", "Select the native artifact kind generated by the test backend.");
    ...
    addArg(HelpOptionGroup::Target, "test", "--native-artifact-base-name", nullptr, CommandLineType::String, &cmdLine_->nativeArtifactBaseName, nullptr, "Set the native artifact base name.");
    addArg(HelpOptionGroup::Target, "test", "--native-artifact-output-dir", nullptr, CommandLineType::Path, &cmdLine_->nativeArtifactOutputDir, nullptr, "Set the native artifact output directory.");
    addArg(HelpOptionGroup::Target, "test", "--native-work-dir-name", nullptr, CommandLineType::String, &cmdLine_->nativeWorkDirName, nullptr, "Set the temporary native work directory name.");
